...
from custom_script import STIR_INITIAL, TEMP_INITIAL, IPP_ADDRESSES, DILUTION_PERIOD
...
class EvolverNamespace(socketio.ClientNamespace):
    ...
    def on_broadcast(self, data):
        ...
        test = False
        if (self.last_custom_function_run == 0 or elapsed_time >= self.last_custom_function_run + DILUTION_PERIOD) and self.robotics_ns.status['mode'] == 'idle':
            logger.info('running custom functions')
            self.last_custom_function_run = elapsed_time
    ...
    def transform_data(self, data, quads, od_cal, temp_cal):
        ...
        for quad in range(len(quads)):
            ...
            for vial in quads[quad_key]:
                file_name =  "quad{0}_vial{1}-temp_config.txt".format(quad,vial)
    ...
    def _create_file(self, vial, quad, param, directory=None, defaults=None):
        ...
        if directory is None:
            directory = param
        current_quad = int(quad.split("_")[1])
        file_name =  "quad{0}_vial{1}-{2}.txt".format(current_quad, vial, param)
    ...
    def initialize_exp(self, quads, robotics_ns, experiment_params, log_name, quiet, verbose, ip_address, always_yes = False):
        ...
        if exp_continue == 'n':
            ...
            for quad in quads:
                ...
                os.makedirs(os.path.join(EXP_DIR, quad, 'syringe_pump_log'))
                ...
                for vial in quads[quad]:
                    ...
                    self._create_file(vial, quad, 'temp_config', defaults=[exp_str, "0,{0}".format(TEMP_INITIAL[current_quad])])
                    # make syringe pump log file
                    self._create_file(vial, quad, 'syringe_pump_log', defaults=[exp_str, "0,0"])
                    # make ODset file
    ...
    def calc_growth_rate(self, quad, vial, gr_start, elapsed_time):
        ODfile_name =  "quad{0}_vial{0}-OD.txt".format(quad, vial)
        # Grab Data and make setpoint
        OD_path = os.path.join(EXP_DIR, 'quad_{0}'.format(quad), 'OD', ODfile_name)
        ...
        logger.debug('growth rate for vial %s in quad %s: %.2f' % (vial, quad, slope))

        # Save slope to file
        file_name =  "quad{0}_vial{1}_gr.txt".format(quad, vial)
        gr_path = os.path.join(EXP_DIR, 'quad_{0}'.format(quad), 'growthrate', file_name)
    ...
    def custom_functions(self, data, vials, elapsed_time, test):
        ...
        elif mode == 'chemostat_HT':
            custom_script.chemostat_HT(self, data, vials, elapsed_time, test)
        else:
