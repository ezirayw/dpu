...
class RoboticsNamespace(socketio.ClientNamespace):
    ...
    def on_broadcast(self, data):
        ...
        if self.status["mode"] == "idle" and not self.running_routine:
    ...
    def pause_experiment(self):
        ...
        self.emit("pause_robotics", {}, namespace="/robotics_evolver")

    def resume_experiment(self):
        self.logger.info("resuming experiment")
        self.emit("resume_robotics", {}, namespace="/robotics_evolver")

    def stop_experiment(self):
        self.logger.info("stopping experiment")
        self.emit("stop_robotics", {}, namespace="/robotics_evolver")

    def exit_experiment(self):
        self.logger.info("exiting experiment")
        self.emit("exit_robotics", {}, namespace="/robotics_evolver")
    ...
    def pipette(self, data):
        ...
        self.request_robotics_status()
        while not self.retreive_status:
            time.sleep(0.1)
        ...
        if self.status["mode"] == "idle":
            ...
            self.emit(
                ...
                callback=self.acknowledge_routine,
            )
        else:
    ...
    def prime_influx_pumps(self, syringe_pumps={}):
        ...
        if self.status["mode"] == "idle":
            ...
            self.emit(
                "prime_influx_routine", syringe_pumps, namespace="/robotics_evolver"
            )
        else:
    ...
    def prime_efflux_pumps(self, quads):
        ...
        if self.status["mode"] == "idle":
            ...
            self.emit("prime_efflux_routine", data, namespace="/robotics_evolver")
        else:
    ...
    def fill_vials_syringe_pumps(self, fluidic_commands, quads):
        ...
        if self.status["mode"] == "idle":
            self.running_routine = True
            self.logger.info(
                "fill vials media and/or other fluids via syringe pumps: %s"
                % fluidic_commands
            )
            print(
                "setup vials with media prior to innoculation: {}".format(
                    fluidic_commands
                )
            )
            data = {
            ...
            self.logger.warning(
                "robotics not in idle mode, cannot fill vials with robotic xArm"
            )
    ...
    def fill_vials_ipp(self, quads):
        ...
        if self.status["mode"] == "idle":
            self.running_routine = True
            self.logger.info(
                "fill vials with media and/or other fluids via efflux IPPs"
            )
            ...
            self.logger.warning(
                "robotics not in idle mode, cannot fill vials with efflux IPPs"
            )
    ...
    def request_pump_settings(self):
        ...
        self.emit(
            ...
            {},
            namespace="/robotics_evolver",
            callback=self.acknowledge_retreival,
        )
    ...
    def override_status(self, data):
        ...
        self.emit("override_robotics_status", data, namespace="/robotics_evolver")
    ...
    def reconnect(self, data):
        ...
        self.emit("reconnect_robotics", data, namespace="/robotics_evolver")
