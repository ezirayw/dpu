...
def turbidostat_HT(eVOLVER, input_data, quads, elapsed_time, test):
    ...
    if SYRINGE_PUMP_MESSAGE != []:
        ...
        if (eVOLVER.robotics_ns.status['mode'] == 'idle'): # if robotics are idle, ready to receive influx command
            eVOLVER.robotics_ns.start_dilutions(fluidic_commands, active_quads, test=test)
