...
QUADS = {
    "quad_0": [x for x in range(18)],
    "quad_1": [x for x in range(18)],
    "quad_2": [x for x in range(18)],
    "quad_3": [x for x in range(18)],
}
...
class EvolverNamespace(socketio.ClientNamespace):
                ...
                if fit["active"]:
                    ...
                    for param in fit["params"]:
                        for quad in QUADS:
                            ...
                            if not os.path.isdir(os.path.join(EXP_DIR, quad, param + "_raw")) and (
                                param != "pump" or param != "syringe_pump" or param != "ipp"
                            ):
    ...
    def request_calibrations(self):
        logger.debug("requesting calibrations")
        self.emit("getcalibration", {}, namespace="/dpu-evolver")

    def receive_calibrations(self, data):
        if data.get("calibration") is not "error":
            logger.debug("receiving calibrations")
    ...
    def transform_data(self, data, quads, od_cal, temp_cal):
        ...
        for quad in range(len(od_data)):
            od_data[quad].extend(
                [
                    float(od_data_left[quad][0]),
                    float(od_data_left[quad][1]),
                    float(od_data_left[quad][2]),
                    float(od_data_right[quad][0]),
                    float(od_data_right[quad][1]),
                    float(od_data_right[quad][2]),
                ]
            )
            od_data[quad].extend(
                [
                    float(od_data_left[quad][3]),
                    float(od_data_left[quad][4]),
                    float(od_data_left[quad][5]),
                    float(od_data_right[quad][3]),
                    float(od_data_right[quad][4]),
                    float(od_data_right[quad][5]),
                ]
            )
            od_data[quad].extend(
                [
                    float(od_data_left[quad][6]),
                    float(od_data_left[quad][7]),
                    float(od_data_left[quad][8]),
                    float(od_data_right[quad][6]),
                    float(od_data_right[quad][7]),
                    float(od_data_right[quad][8]),
                ]
            )
            ...
            for vial in quads[quad_key]:
                ...
                try:
                    if od_cal["type"] == SIGMOID:
                        ...
                        od_data[quad][vial] = np.real(
                            od_coefficients[2]
                            - (
                                (
                                    np.log10(
                                        (od_coefficients[1] - od_coefficients[0]) / (float(od_data[quad][vial]) - od_coefficients[0]) - 1
                                    )
                                )
                                / od_coefficients[3]
                            )
    ...
    def stop_all_pumps(self):
        ...
        data = {"param": "pump", "value": value, "recurring": False, "immediate": True}
    ...
    def initialize_exp(self, quads, robotics_ns, experiment_params, log_name, quiet, verbose, ip_address, always_yes=False):
        ...
        shutil.copy(os.path.join(SAVE_PATH, "custom_script.py"), os.path.join(EXP_DIR, backup_filename))
    ...
    def check_for_calibrations(self):
        ...
        if (
            ...
            or not os.path.exists(SYRINGE_PUMP_CAL_PATH)
            or not os.path.exists(IPP_CAL_PATH)
        ):
    ...
    def calc_growth_rate(self, vial, gr_start, elapsed_time):
        ...
        slope, intercept, r_value, p_value, std_err = stats.linregress(trim_time[np.isfinite(log_OD)], log_OD[np.isfinite(log_OD)])
...
def setup_logging(filename, quiet, verbose):
    if quiet:
        ...
        if verbose == 0:
            ...
            level = logging.DEBUG
        logging.basicConfig(
            format="%(asctime)s - %(name)s - [%(levelname)s] - %(message)s", datefmt="%Y-%m-%d %H:%M:%S", filename=filename, level=level
        )
...
def get_options():
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="Answer yes to all questions (i.e. continues from existing experiment, overwrites existing data and blanks OD measurements)",
    )
    parser.add_argument(
        "-l", "--log-name", default=os.path.join(EXP_DIR, "evolver.log"), help="Log file name directory (default: %(default)s)"
    )
    parser.add_argument("-i", "--ip-address", action="store", dest="ip_address", help="IP address of eVOLVER to run experiment on.")

    log_nolog = parser.add_mutually_exclusive_group()
    log_nolog.add_argument("-v", "--verbose", action="count", default=0, help="Increase logging verbosity level to DEBUG (default: INFO)")
    log_nolog.add_argument("-q", "--quiet", action="store_true", default=False, help="Disable logging to file entirely")
...
if __name__ == "__main__":
    ...
    EVOLVER_NS.start_time = EVOLVER_NS.initialize_exp(
        QUADS, ROBOTICS_NS, experiment_params, options.log_name, options.quiet, options.verbose, evolver_ip, options.always_yes
    )
